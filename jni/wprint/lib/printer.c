...
static const ifc_print_job_t _print_job_ifc =
{
    ...
    .end_job        = _end_job,
    .destroy        = _destroy,
    .enable_timeout = _enable_timeout,
...
void printer_get_timeout(long int *timeout_msec)
{
    if (timeout_msec != NULL)
        *timeout_msec = _wprint_timeout_msec;
}

void printer_set_timeout(long int timeout_msec)
{
    if (timeout_msec > 0)
        _wprint_timeout_msec = timeout_msec;
}
...
_print_job_t  *print_job;
    ...
    /*  if it is for IPP, get it from its connect function  */
    // if (port_num == PORT_IPP)
        // return(ipp_get_print_ifc());
...
_print_job_t  *print_job = IMPL(_print_job_t, ifc, this_p);
   ...
   {
       print_job->psock = open(printer_addr,
                               O_CREAT | O_WRONLY | O_TRUNC, 
                               S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);

       if (print_job->psock == ERROR)
       {
           ifprint((DBG_ERROR, "cannot create output file : %s, %s", 
                    printer_addr, strerror(errno)));
       }
       else
       {
           ifprint((DBG_IO, "opened %s for writing", printer_addr));
       }
   }
   else   /*  open a socket to the printer:port */
   {
...
_print_job_t  *print_job = IMPL(_print_job_t, ifc, this_p);
   ...
   if (this_p && buffer && (print_job->job_status == OK))
   {
       ...
       else
       {
           ...
           while((length > 0) && (retval == OK))
           {
               ...
               if (selreturn < 0)
               {
                   ifprint((DBG_ERROR, "select returned an errnor (%d)", errno));
                   retval = ERROR;
               }
               else if (selreturn > 0)
               {
                   if (FD_ISSET(print_job->psock, &w_fds))
                   {
                       ...
                       if (bytes_written < 0)
                       {
                           ifprint((DBG_ERROR, "unable to transmit %d bytes of data (errno %d)", length, errno));
                           retval = ERROR;
                       }
                       else
                       {
                   ...
                   {
                       ifprint((DBG_ERROR, "select returned OK, but fd is not set"));
                       retval = ERROR;
                   }
               }
               else
               {
                   ...
                   if (retval == ERROR)
                       ifprint((DBG_ERROR, "select timed out"));
               }
           }
       }
   ...
   return((retval == OK) ? length_in : ERROR);
...
int    psock;
    ...
    if ( (sin.sin_addr.s_addr = inet_addr(printer_addr)) == -1 )
    {
        ...
        {
            (void) memcpy(&(sin.sin_addr.s_addr), h_info->h_addr, 
                          h_info->h_length);
        }
        else
        {
            ifprint((DBG_ERROR, "ERROR: unknown host %s", printer_addr));
            close(psock);
            return(ERROR);
        }
    }
    ...
    if(socketConnect == 0)	// 20120302 - LK - fixed coverity issue by checking return value from connect()
    {
		FD_ZERO(&fdset);
		FD_SET(psock, &fdset);
		...
		if (select(psock + 1, NULL, &fdset, NULL, &tv) == 1)
		{
			...
			if (so_error == 0)
			{
			   ...
			   ifprint((DBG_IO, "connected to %s:%d", printer_addr, port_num));
			}
			else
			{
			   close(psock);
			   psock = ERROR;
			   ifprint((DBG_ERROR, "ERROR: cannot connect on %s:%d, %s",
						printer_addr, port_num, strerror(errno)));
			}
		}
		else
		{
			 ifprint((DBG_ERROR,
					  "ERROR: connecting to %s:%d .. timed out after %d milliseconds",
					  printer_addr, port_num, timeout_msec));
			 close(psock);
			 psock = ERROR;
		}
    }
