...
static void *_send_thread(void *param)
{
    ...
    while(priv->job_info.wprint_ifc->msgQReceive(priv->msgQ, (char*)&msg, sizeof(msgQ_msg_t), WAIT_FOREVER) == OK)
    {
        if (msg.id == MSG_START_JOB)
        {
            priv->pcl_ifc->start_job( priv->job_handle,
                                      &priv->job_info,
                                      priv->job_params->media_size,
                                      priv->job_params->media_type,
                                      priv->job_params->print_quality,
                                      priv->job_params->pixel_units,
                                      priv->job_params->duplex,
                                      priv->job_params->dry_time,
                                      priv->job_params->color_space,
                                      priv->job_params->mono1Bit,
                                      priv->job_params->media_tray,
                                      priv->job_params->page_top_margin,
                                      priv->job_params->page_left_margin );
        }
        else if (msg.id == MSG_START_PAGE)
        {
                priv->pcl_ifc->start_page( &priv->job_info,
                                           msg.param.start_page.extra_margin,
                                           msg.param.start_page.width,
                                           msg.param.start_page.height );
        }
        else if (msg.id == MSG_SEND)
        {
            if (!priv->pcl_ifc->canCancelMidPage() || !priv->job_params->cancelled)
                priv->pcl_ifc->print_swath(&priv->job_info,
                                           msg.param.send.buffer, 
                                           msg.param.send.start_row,
                                           msg.param.send.num_rows,
...
static int _start_thread(plugin_data_t *param)
{
...
int result;
   ...
   if (result == OK)
   {
       result = pthread_create(&(param->send_tid), 0, _send_thread, (void*)param);
...
static int _stop_thread(plugin_data_t *priv)
{
    int result = ERROR;
    if (priv == NULL)
        return(result);
    if (!pthread_equal(priv->send_tid, pthread_self()))
    {
        ...
        priv->job_info.wprint_ifc->msgQSend(priv->msgQ, (char*)&msg, sizeof(msgQ_msg_t), NO_WAIT, MSG_Q_FIFO);
                         ...
                         wprint_job_params_t   *job_params )
{
    ...
    do
    {
        ...
        {
            priv->job_info.wprint_ifc->debug(DBG_ERROR, "ERROR: cannot start PCL job, no ifc found");
            continue;
        }

        priv->msgQ = priv->job_info.wprint_ifc->msgQCreate((MAX_SEND_BUFFS * 2), sizeof(msgQ_msg_t), MSG_Q_FIFO);
        if (priv->msgQ == MSG_Q_INVALID_ID)
            continue;

        if (_start_thread(priv) == ERROR)
            continue;
...
msgQ_msg_t msg;
    ...
    if (pathname == NULL)
    {
        priv->job_info.wprint_ifc->debug(DBG_ERROR, "_print_page(): cannot print file with NULL name");
    ...
    else if (strlen(pathname))
    {
        ...
        if (imgfile)
        {
           priv->job_info.wprint_ifc->debug(DBG_LOG, "_print_page(): fopen succeeded on %s", pathname);
           ...
           if ( (result = wprint_image_get_info(imgfile, image_info)) == OK )
           {
              ...
              if ((job_params->render_flags & RENDER_FLAG_PORTRAIT_MODE) != 0) {
                  priv->job_info.wprint_ifc->debug(DBG_LOG, "_print_page(): portrait mode");
                  rotation = ROT_0;
              } else if ((job_params->render_flags & RENDER_FLAG_LANDSCAPE_MODE) != 0) {
                  priv->job_info.wprint_ifc->debug(DBG_LOG, "_print_page(): landscape mode");
                  rotation = ROT_90;
              } else if (wprint_image_is_landscape(image_info) &&
                  ((job_params->render_flags & RENDER_FLAG_AUTO_ROTATE) != 0)) {
                  priv->job_info.wprint_ifc->debug(DBG_LOG, "_print_page(): auto mode");
                  rotation = ROT_90;
              }
              if ((job_params->render_flags & RENDER_FLAG_CENTER_ON_ORIENTATION) != 0) {
                  job_params->render_flags &= ~(RENDER_FLAG_CENTER_HORIZONTAL | RENDER_FLAG_CENTER_VERTICAL);
                  job_params->render_flags |= ((rotation == ROT_0) ? RENDER_FLAG_CENTER_HORIZONTAL : RENDER_FLAG_CENTER_VERTICAL);
              }
              ...
              priv->job_info.wprint_ifc->debug(DBG_LOG, "_print_page(): rotation = %d", rotation);

              wprint_image_set_output_properties(image_info,
                                                 rotation,
                                                 job_params->printable_area_width,
                                                 job_params->printable_area_height,
                                                 job_params->print_top_margin,
                                                 job_params->print_left_margin,
                                                 job_params->print_right_margin,
                                                 job_params->print_bottom_margin,
                                                 job_params->render_flags,
                                                 job_params->strip_height, MAX_SEND_BUFFS, image_padding);
              ...
              if (i == MAX_SEND_BUFFS)
              {
                ...
                msg.param.start_page.extra_margin = ((job_params->duplex != DF_DUPLEX_MODE_NONE) &&
                ...
                while ( (result != ERROR) && (num_rows > 0) )
                {
                  ...
                  if (priv->pcl_ifc->canCancelMidPage() && job_params->cancelled)
                      break;
                  //coverity[atomicity] : wait for a buffer to be available
                  ...
                  if (nbytes > 0)
                  {
                     ...
                     msg.param.send.start_row = start_row;
                     msg.param.send.num_rows = height;

                     result = priv->job_info.wprint_ifc->msgQSend(priv->msgQ, (char*)&msg, sizeof(msgQ_msg_t), NO_WAIT, MSG_Q_FIFO);
                     if (result < 0)
                     {
                         //coverity[atomicity] : release semaphore since the send did not go through and can't release the semaphore
                         sem_post(&priv->buffs_sem);
                     }
                  ...
                  {
                      //coverity[atomicity] : release semaphore since there is no send to release the semaphore
                      sem_post(&priv->buffs_sem);
                      if (nbytes < 0)
                      {
                          priv->job_info.wprint_ifc->debug(DBG_ERROR, "_print_page(): ERROR: file appears to be corrupted");
                          result = CORRUPT;
                      }
                      break;
                  }
                ...
                priv->job_info.wprint_ifc->debug(DBG_LOG, "_print_page(): sends done, result: %d", result);

                /*  free the buffer and eject the page  */

                msg.param.end_page.page = job_params->page_num;
                priv->job_info.wprint_ifc->debug(DBG_LOG,
                                 "_print_page(): processed %d out of %d rows of page # %d from %s to printer %s %s {%s}",
                                 start_row, wprint_image_get_height(image_info), job_params->page_num, pathname,
              ...
              else
              {
                  msg.param.end_page.page = -1;
                  result = ERROR;
                  priv->job_info.wprint_ifc->debug(DBG_ERROR, "_print_page(): ERROR: plugin_pcl cannot allocate memory for image stripe");
              }
              for(i  = 0; i < MAX_SEND_BUFFS; i++)
           ...
           else
           {
              ...
              msg.param.end_page.count = 0;
              result = CORRUPT;
              priv->job_info.wprint_ifc->debug(DBG_ERROR, "_print_page(): ERROR: file does not appear to be valid");
           }
        ...
        else
        {
            msg.param.end_page.page = -1;
            msg.param.end_page.count = 0;
            priv->job_info.wprint_ifc->debug(DBG_ERROR, "_print_page(): ERROR: could not open %s", pathname);
            result = CORRUPT;
        }
        free(image_info);
    }
    ...
    {
        priv->job_info.wprint_ifc->debug(DBG_ERROR, "_print_page(): ERROR: filename was empty");
        msg.param.end_page.page = -1;
        msg.param.end_page.count = 0;
        result = ERROR;
    }
...
static int _print_blank_page ( wJob_t                job_handle,
                               wprint_job_params_t   *job_params )
{
    msgQ_msg_t msg;
    plugin_data_t *priv;
...
{
static const wprint_plugin_t _pcl_plugin = 
  {
     ...
     .print_blank_page  = _print_blank_page,
     .end_job           = _end_job,
   };
