...
/*____________________________________________________________________________
 ...
 |____________________________________________________________________________|
*/
uint32 _scaler_fraction_part    (uint32           iNum,
                                 uint32           iDen,
				 pscaler_fraction_t mode,
				 bool_t          *overflow);
static void _hw_scale_image_plane(scaler_config_t *pscaler_config,
	scaler_mode_t scaleMode);

static void _calculate_factors (scaler_config_t* pscaler_config, scaler_mode_t scaleMode);
/*____________________________________________________________________________
 |                                                                            |
 | Private-to-this-Component Data                                             |
 |____________________________________________________________________________|
*/

// called once per ip job
// stores image dimensions
// stores mem pool handle
// determins the scale mode (UP, DOWN, MIXED)
// calculates scaler factors,
// calculates initial buffer sizes for scaling whole image
//  start rows    == 0
//  end_rows      == image height
//  buffer widths == image widths
//  strip sizes   == image width * image height
//  temp buf size == largest width * largest height (if needed, 0 if not)
void LIB_PHOTO_SCALER_make_image_scaler_tables(uint16 image_input_width, 
                                           ...
                                           scaler_config_t* pscaler_config)
{
    ...
    if ((image_input_width >= image_output_width) &&
	    (image_input_height >= image_output_height))
    { // scale DOWN
    ...
    { // scale UP
        ...
        if (image_input_width > image_output_width)
    { // mixed scale Y-axis first
        pscaler_config->scaleMode      = PSCALER_SCALE_MIXED_YUP;
    }
    else
    { // mixed scale X-axis first
...
} // end scaler_setup_config





// called AFTER scaler_setup_config
// called once per image stripe/slice
// stores output start & end
// calculates source start and end rows
// calculates stripe sizes
void LIB_PHOTO_SCALER_calculate_scaling_rows(uint16 start_output_row_number,
                                         ...
                                         uint32 *mixed_axis_temp_buffer_size_needed)
{
	...
	*num_rows_offset_to_start_output_row = 0;
    ...
    if (pscaler_config->iOutEndRow >= pscaler_config->iOutHeight)
    { // last stripe
	...
	(uint32)pscaler_config->iOutStartRow *
	    ...
	    pscaler_config->scaleMode == PSCALER_SCALE_MIXED_YUP)
    {
		...
		(uint32)pscaler_config->iOutEndRow *
		(uint32)pscaler_config->iSrcHeight,
		(uint32)pscaler_config->iOutHeight,
		FRACTION_TRUNCATE,
		&overflow);
        ...
        {   // will cause an extra output row to be created...
            ...
            // does iOutEndRow need to be incrementeded as well??
            pscaler_config->iOutEndRow++; // TRY IT!?
        }
    ...
    else
    {
        ...
        fSrcEndRow.fraction = _scaler_fraction_part (
		(uint32)(pscaler_config->iOutEndRow + 1) *
		(uint32)pscaler_config->iSrcHeight,
		(uint32)pscaler_config->iOutHeight,
		FRACTION_TRUNCATE,
		&overflow);
    ...
    if (pscaler_config->iSrcEndRow >= pscaler_config->iSrcHeight)
    { // last stripe
    ...
    *num_output_rows_generated = (pscaler_config->iOutEndRow - 
	...
	*mixed_axis_temp_buffer_size_needed =
    ...
    {
	*mixed_axis_temp_buffer_size_needed =
	    ROUND_4_UP(pscaler_config->iSrcWidth) *
	    (*num_output_rows_generated + 1);
    }
    else
    {
...
//  REVISIT: Scaler needs an EXTRA output row for MOST stripes
//   This will be fixed in the BONES asic
    (*num_output_rows_generated)++;
       
} // end scaler_calculate_rows


void LIB_PHOTO_SCALER_scale_image_data(uint8   *input_plane,
                                       uint16 start_input_row_number,
                                       uint16 end_input_row_number,
                                       ...
                                       uint8  *temp_buffer_for_mixed_axis_scaling)
{
	...
	pscaler_config->pTmpBuf = temp_buffer_for_mixed_axis_scaling;
        ...
        } // end if (PSCALER_SCALE_MIXED_YUP == pscaler_config->scaleMode)
        else
        // if (PSCALER_SCALE_MIXED_XUP == pscaler_config->scaleMode)
        {
...
//	    fOrigSrcStartRow.decimal   = pscaler_config->fSrcStartRow.decimal
        ...
        } // end if (PSCALER_SCALE_MIXED_XUP == pscaler_config->scaleMode)
...
} // end scale_image_plane


// returns total mem needed per 1 output row to scale the image
//  CALLER MUST anticipiate an EXTRA output row for MOST stripes
//   This will be fixed in the BONES asic
uint32 scaler_query_mem_needed (uint16  src_w, uint16  src_h,
	uint16  out_w, uint16  out_h,
	uint32* out_mem_needed,
	uint32* tmp_mem_needed)
{

    if ((src_w >= out_w) && (src_h >= out_h))
    { // Down only
        *out_mem_needed = out_w;
        *tmp_mem_needed = 0;
    }
    else
    ...
    { // UP in the HEIGHT
        *out_mem_needed = out_w;
        *tmp_mem_needed = src_w * (out_h / src_h + 1);
    }
    else
    //if (src_w < out_w)
    {
        // UP in the WIDTH
        *out_mem_needed = out_w;
        *tmp_mem_needed = out_w * (src_h / out_h + 1);
    }

    return(*out_mem_needed + *tmp_mem_needed);
} // end scaler_query_mem_needed
...
} // end PHOTO_SCALER_calculate_factors

// Code from V.Z.
#define PSCALER_FRACT_BITS_COUNT 24
uint32 _scaler_fraction_part (uint32 iNum, uint32 iDen,
	pscaler_fraction_t mode,
	bool_t          *overflow)
{
...
} // end _scaler_fraction_part


volatile int fill_buf = 0;
#define _RESTRICT_ __restrict__

static inline void _scale_row_down_9in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ in3,
                                uint8 * _RESTRICT_ in4,
                                uint8 * _RESTRICT_ in5,
                                uint8 * _RESTRICT_ in6,
                                uint8 * _RESTRICT_ in7,
                                uint8 * _RESTRICT_ in8,
                                uint8 * _RESTRICT_ out,
                                uint64 position_x,
                                uint64 x_factor_inv,
                                uint32 top_weight,
                                uint32 bot_weight,
                                uint32 weight_reciprocal,
                                int out_width)
{
  int x;
  uint32 in_col;
...
static inline void _scale_row_down_8in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ in3,
                                uint8 * _RESTRICT_ in4,
...
static inline void _scale_row_down_7in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ in3,
                                uint8 * _RESTRICT_ in4,
...
static inline void _scale_row_down_6in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ in3,
                                uint8 * _RESTRICT_ in4,
...
static inline void _scale_row_down_5in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ in3,
                                uint8 * _RESTRICT_ in4,
...
static inline void _scale_row_down_4in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ in3,
                                uint8 * _RESTRICT_ out,
...
static inline void _scale_row_down_3in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ in2,
                                uint8 * _RESTRICT_ out,
                                uint64 position_x,
...
static inline void _scale_row_down_2in(uint8 * _RESTRICT_ in0,
                                uint8 * _RESTRICT_ in1,
                                uint8 * _RESTRICT_ out,
                                uint64 position_x,
                                uint64 x_factor_inv,
...
static inline void _scale_row_down(uint8 * in,
                            uint8 * _RESTRICT_ out, 
                            uint32 in_row_ofs,
                            uint64 position_x, 
                            uint64 position_y, 
  ...
  else
  {
    for (x=0;x<out_width;x++)
    {
      ...
      while (total_weight > 0)
      {
        ...
        acc_r += (uint32) in[y*in_row_ofs + ((in_col * 3) + 0)]*curr_weight*bot_weight;
        acc_g += (uint32) in[y*in_row_ofs + ((in_col * 3) + 1)]*curr_weight*bot_weight;
        acc_b += (uint32) in[y*in_row_ofs + ((in_col * 3) + 2)]*curr_weight*bot_weight;
...
static void _scale_row_up(uint8 * _RESTRICT_ in0,
                          uint8 * _RESTRICT_ in1, 
                          uint8 * _RESTRICT_ out, 
                          sint32 weight_y,
                          uint64 position_x, 
                          ...
                          int out_width)
{
  ...
  for (x=0;x<out_width;x++)
  {
    ...
    top_val_r = (in0[(pix_x * 3) + 0] << 10) + weight_x*((sint32) in0[((pix_x + 1) * 3) + 0]-in0[(pix_x * 3) + 0]);
    bot_val_r = (in1[(pix_x * 3) + 0] << 10) + weight_x*((sint32) in1[((pix_x + 1) * 3) + 0]-in1[(pix_x * 3) + 0]);

    top_val_g = (in0[(pix_x * 3) + 1] << 10) + weight_x*((sint32) in0[((pix_x + 1) * 3) + 1]-in0[(pix_x * 3) + 1]);
    bot_val_g = (in1[(pix_x * 3) + 1] << 10) + weight_x*((sint32) in1[((pix_x + 1) * 3) + 1]-in1[(pix_x * 3) + 1]);

    top_val_b = (in0[(pix_x * 3) + 2] << 10) + weight_x*((sint32) in0[((pix_x + 1) * 3) + 2]-in0[(pix_x * 3) + 2]);
    bot_val_b = (in1[(pix_x * 3) + 2] << 10) + weight_x*((sint32) in1[((pix_x + 1) * 3) + 2]-in1[(pix_x * 3) + 2]);
                                  ...
                                  scaler_mode_t    scaleMode)
{
  ...
  outp = ((uint8 *) pscaler_config->pOutBuf) + (first_xi * 3);
  ...
  {
    uint8 *inp  = ((uint8 *) pscaler_config->pSrcBuf) + 
