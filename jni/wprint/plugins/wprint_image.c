...
void wprint_image_setup( wprint_image_info_t *image_info,
...
int wprint_image_get_info( FILE                  *imgfile,
                           wprint_image_info_t   *image_info )
{
    ...
    if ((decode_ifc != NULL) && (decode_ifc->get_hdr != NULL))
    {
        ...
        {
            image_info->wprint_ifc->debug(DBG_LOG,
                  "wprint_image_get_info(): %s dim = %dx%d", image_info->mime_type, image_info->width,image_info->height);
            return OK;
        }
        else
        {
            image_info->wprint_ifc->debug(DBG_ERROR,
                                          "ERROR: get_hdr for %s", image_info->mime_type);
            return(ERROR);
        }
    }
    image_info->wprint_ifc->debug(DBG_ERROR,
                                "Unsupported image type: %s", image_info->mime_type);
    return(ERROR);
}   /*  wprint_image_get_info  */

int wprint_image_set_output_properties( wprint_image_info_t *image_info,
                                        ...
                                        unsigned int        padding_options)
{
    ...
    {
        image_info->wprint_ifc->debug(DBG_LOG, "calculating subsampling");
        const image_decode_ifc_t *decode_ifc = image_info->decode_ifc;
        ...
        image_info->wprint_ifc->debug(DBG_LOG, "calculated sample size: %d", image_info->scaled_sample_size);
        ...
        if ((image_output_width == image_info->printable_width) &&
            (((image_info->render_flags & RENDER_FLAG_AUTO_SCALE) &&
             ...
             ((image_info->render_flags & RENDER_FLAG_AUTO_FIT) &&
              (image_output_height < image_info->printable_height))))
        {
            image_info->wprint_ifc->debug(DBG_LOG, "disabling fine scaling since width matches and height meets criteria");
            image_info->render_flags &= ~(RENDER_FLAG_AUTO_SCALE | RENDER_FLAG_AUTO_FIT);
        }
        // check if height matches and width meets criteria
                 ...
                 (((image_info->render_flags & RENDER_FLAG_AUTO_SCALE) &&
                  ...
                  ((image_info->render_flags & RENDER_FLAG_AUTO_FIT) &&
                   (image_output_width >= image_info->printable_width))))
        {
            image_info->wprint_ifc->debug(DBG_LOG, "disabling fine scaling since height matches and width meets criteria");
            image_info->render_flags &= ~(RENDER_FLAG_AUTO_SCALE | RENDER_FLAG_AUTO_FIT);
        }
    }
    ...
    if (image_info->render_flags & (RENDER_FLAG_AUTO_SCALE | RENDER_FLAG_AUTO_FIT))
    {
        image_info->wprint_ifc->debug(DBG_LOG, "calculating fine-scaling");
        int i;
         ...
         */
        LIB_PHOTO_SCALER_make_image_scaler_tables(image_output_width,
                                                  BYTES_PER_PIXEL(image_output_width),
                                                  image_info->scaled_width,
                                                  BYTES_PER_PIXEL(image_info->scaled_width),
                                                  image_output_height,
                                                  image_info->scaled_height,
                                                  &image_info->scaler_config);
        ...
        for(i = 0; i < image_info->printable_height; i += max_decode_stripe)
        {
            ...
            if (row >= image_info->scaled_height)
                break;
            LIB_PHOTO_SCALER_calculate_scaling_rows(row,
        ...
        int unscaled_size = BYTES_PER_PIXEL((MAX(image_output_width, image_output_height) * image_info->unscaled_rows_needed));
        ...
        image_info->mixed_memory  = (image_info->mixed_memory_needed != 0) ? malloc(image_info->mixed_memory_needed) : NULL;
    }
    else
    {
    ...
    if (image_info->render_flags & (RENDER_FLAG_AUTO_SCALE | RENDER_FLAG_AUTO_FIT))
    {
        /* use the full image size since both of the dimensions could be greater than the printable area */
    ...
    switch(image_info->rotation)
    {
        case ROT_90:
            ...
            else if (image_info->render_flags & RENDER_FLAG_CENTER_HORIZONTAL)
            {
                ...
                {
                    image_info->col_offset = ((image_info->scaled_width - image_info->printable_width) / 2);
                }
                else
                {
            ...
            {
                image_info->output_padding_right += (image_info->printable_width - image_info->scaled_width);
            }
            ...
            else if (image_info->render_flags & RENDER_FLAG_CENTER_VERTICAL)
            {
                ...
                {
                    image_info->row_offset = ((image_info->scaled_height - image_info->printable_height) / 2);
                }
                else
                {
                ...
                image_info->output_padding_bottom += (image_info->printable_height - image_info->scaled_height);
            }
        ...
        case ROT_180:
            ...
            {
                image_info->output_padding_left += (image_info->printable_width - image_info->scaled_width);
            }
            ...
            {
                image_info->output_padding_top += (image_info->printable_height - image_info->scaled_height);
            }
    ...
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): input render flags - %d (0x%8.8x)", input_render_flags, input_render_flags);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): printable area - %dx%d", printable_width, printable_height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): input margins: Top:%d Left:%d Right:%d Bottom:%d", top_margin, left_margin, right_margin, bottom_margin);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): padding options: %d (0x%2.2x)", image_info->padding_options, image_info->padding_options);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): concurrent stripes - %d", image_info->concurrent_stripes);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): stripe height - %d", image_info->stripe_height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): image dimensions: %dx%d", image_info->width, image_info->height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): image rotation: %d", image_info->rotation);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): final render flags - %d (0x%8.8x)", image_info->render_flags, image_info->render_flags);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): printable area after margins - %dx%d", image_info->printable_width, image_info->printable_height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): output_padding: Top:%d Left:%d Right:%d Bottom:%d", image_info->output_padding_top, image_info->output_padding_left, image_info->output_padding_right , image_info->output_padding_bottom);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): output dimensions: %dx%d", image_info->output_width, image_info->output_height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): subsampled image dimensions - %dx%d", image_info->sampled_width, image_info->sampled_height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): scaled image dimensions - %dx%d", image_info->scaled_width, image_info->scaled_height);
    image_info->wprint_ifc->debug(DBG_LOG,
                                 "wprint_image_set_output_properties(): image offsets - row: %d, col: %d", image_info->row_offset, image_info->col_offset);
    image_info->wprint_ifc->debug(DBG_LOG,
                                  "wprint_image_set_output_properties(): margins - top: %d, left: %d, right: %d, bottom: %d",
    ...
    return OK;
} /* wprint_image_set_output_properties */
...
static int _get_width( wprint_image_info_t *image_info, unsigned int padding_options )
{
    ...
    } else if (image_info->render_flags & RENDER_FLAG_AUTO_FIT) {
...
int wprint_image_get_width( wprint_image_info_t *image_info )
{
    ...
    image_info->wprint_ifc->debug(DBG_LOG, "wprint_image_get_width(): %d", width);
    return(width);
}

int wprint_image_get_output_buff_size( wprint_image_info_t *image_info )
{
    int width = MAX(MAX(image_info->scaled_width, image_info->scaled_height), _get_width(image_info, image_info->padding_options));
    image_info->wprint_ifc->debug(DBG_LOG, "wprint_image_get_output_buff_size(): %dx%d", width, image_info->output_rows);
...
int wprint_image_get_height( wprint_image_info_t *image_info )
{
    ...
    image_info->wprint_ifc->debug(DBG_LOG, "wprint_image_get_height(): %d", height);
    return(height);
}
...
int _decode_stripe( wprint_image_info_t   *image_info,
                    int                   start_row,
                    int                   num_rows,
                    unsigned int          padding_options,
                    unsigned char         *rgb_pixels )
{
    ...
    // get padding values
    int padding_left   = ((padding_options & PAD_LEFT)   ? BYTES_PER_PIXEL(image_info->output_padding_left)  : 0);
    int padding_right  = ((padding_options & PAD_RIGHT)  ? BYTES_PER_PIXEL(image_info->output_padding_right) : 0);
    ...
    switch(image_info->rotation)
    {
        case ROT_90:
            ...
            while(num_rows > 0)
            {
                ...
                {
                    image_info->wprint_ifc->debug(DBG_ERROR, "ERROR: something wrong in ROT_90 calculations. Erroring out to prevent infinite loop");
                    return(ERROR);
                }
                old_num_rows = num_rows;
                    ...
                    (start_row >= (image_info->output_swath_start + image_info->rows_cached)))
                {
                    ...
                    image_info->output_swath_start = ((start_row / image_info->rows_cached) * image_info->rows_cached);
                    for(image_y = 0; image_y < image_info->sampled_height; image_y++) {
                        ...
                        for(image_x = 0; (image_x < image_info->rows_cached &&
                                         ((image_x + image_info->output_swath_start) < image_info->sampled_width));
                            image_x++) {
                            memcpy(image_info->output_cache[image_x] + BYTES_PER_PIXEL((image_info->sampled_height - image_y - 1)),
                                   image_data + BYTES_PER_PIXEL((image_info->output_swath_start + image_x)),
                                   BYTES_PER_PIXEL(1));
                        }
                    }
                }
                ...
                {
                    memcpy(rgb_pixels + padding_left, image_info->output_cache[image_y - image_info->output_swath_start] + col_offset, rbytes);
                ...
                image_y++, num_rows--)
            {
                ...
                for(image_x = 0; image_x < image_info->output_width; image_x++)
                {
                    memcpy(rgb_pixels + padding_left + BYTES_PER_PIXEL(image_x),
                           image_data + BYTES_PER_PIXEL(image_info->sampled_width - image_x - col_offset - 1),
                           BYTES_PER_PIXEL(1));
                }
                nbytes += rbytes + padding_left + padding_right;
                rgb_pixels += rbytes + padding_left + padding_right;
        ...
        case ROT_270:
            ...
            while(num_rows > 0)
            {
                ...
                if (old_num_rows == num_rows)
                {
                    image_info->wprint_ifc->debug(DBG_ERROR, "ERROR: something wrong in ROT_270 calculations. Erroring out to prevent infinite loop.");
                    return(ERROR);
                }
                old_num_rows = num_rows;
                if ((image_info->output_swath_start == -1) ||
                    ...
                    for(image_y = 0; image_y < image_info->sampled_height; image_y++) {
                        ...
                        for(image_x = 0; ((image_x < image_info->rows_cached) &&
                            ...
                            memcpy(image_info->output_cache[image_x] + BYTES_PER_PIXEL(image_y),
                                   image_data + BYTES_PER_PIXEL(image_info->sampled_width - (image_info->output_swath_start + image_x) - 1),
                                   BYTES_PER_PIXEL(1));
                        }
                    }
                }
        ...
        default:
                ...
                image_y++, num_rows--)
            {
                ...
                if (image_data == NULL)
                {
                    ...
                    image_info->wprint_ifc->debug(DBG_ERROR, "ERROR: received no data for row: %d", image_y);
                    return(ERROR);
                }
...
int wprint_image_decode_stripe( wprint_image_info_t   *image_info,
                                int                   start_row,
                                int                   *height,
                                unsigned char         *rgb_pixels )
{
    ...
    *height = 0;
    ...
    // get padding values
    int padding_left   = ((image_info->padding_options & PAD_LEFT)   ? BYTES_PER_PIXEL(image_info->output_padding_left)  : 0);
    int padding_right  = ((image_info->padding_options & PAD_RIGHT)  ? BYTES_PER_PIXEL(image_info->output_padding_right) : 0);
    int padding_top    = ((image_info->padding_options & PAD_TOP)    ? image_info->output_padding_top                    : 0);
    //int padding_bottom = ((image_info->padding_options & PAD_BOTTOM) ? image_info->output_padding_bottom                 : 0);
    ...
    // handle image padding on bottom
        ...
        int blank_rows = MIN(num_rows, _get_height(image_info, image_info->padding_options) - start_row);
        int bytesToBlank = (blank_rows * bytes_per_row);
        nbytes += bytesToBlank;
        num_rows -= blank_rows;
        *height += blank_rows;
    ...
    if (image_info->render_flags & (RENDER_FLAG_AUTO_SCALE | RENDER_FLAG_AUTO_FIT))
    {
        ...
        while(scaled_num_rows > 0)
        {
            ...
            LIB_PHOTO_SCALER_calculate_scaling_rows(scaled_start_row,
            ...
            {
                image_info->wprint_ifc->debug(DBG_ERROR, "need more memory");
                return(ERROR);
            }
            ...
            if (unscaled_row_start <= image_info->unscaled_end_row)
            {
                ...
                memmove(image_info->unscaled_rows,
                        image_info->unscaled_rows +
                        BYTES_PER_PIXEL(((unscaled_row_start - image_info->unscaled_start_row) * image_info->output_width)),
             ...
             */
            int rowsLeftToDecode = ((image_info->unscaled_end_row - (image_info->unscaled_start_row + predecoded_rows)) + 1);
            if (rowsLeftToDecode > 0) {
                int dbytes = _decode_stripe(image_info,
                                            image_info->unscaled_start_row + predecoded_rows,
                                            rowsLeftToDecode,
                                            PAD_NONE,
                                            (image_info->unscaled_rows + BYTES_PER_PIXEL(predecoded_rows * image_info->output_width)));
                if (dbytes <= 0)
                {
                    if (dbytes < 0) {
                        image_info->wprint_ifc->debug(DBG_ERROR, "couldn't decode rows");
                    }
                    return(dbytes);
                }
            ...
            LIB_PHOTO_SCALER_scale_image_data(image_info->unscaled_rows,
                                              image_info->unscaled_start_row,
                                              image_info->unscaled_end_row,
                                              scaled_start_row,
                                              (void*)&image_info->scaler_config,
                                              (uint8*)rgb_pixels,
                                              image_info->mixed_memory);
            ...
            {
                int delta = 0;
                int pixelsToMove = BYTES_PER_PIXEL(MIN(image_info->scaled_width, image_info->printable_width));

                int memMoveRow       = ((bytes_per_row < image_info->scaler_config.iOutBufWidth) ? 0 : (stripe_height - 1));
                int memMoveIncrement = ((bytes_per_row < image_info->scaler_config.iOutBufWidth) ? 1 : -1);
                
                /* if the scaled width is greater than the printable area drop pixels on either size */
                if (image_info->scaled_width > image_info->printable_width) {
                    delta = BYTES_PER_PIXEL(((image_info->scaled_width - image_info->printable_width) / 2));
                }

                /* move the data into the correct location in the output buffer */
                for(generated_rows = 0; generated_rows < stripe_height; generated_rows++, memMoveRow += memMoveIncrement)
                {
                    memmove(rgb_pixels + (memMoveRow * bytes_per_row) + padding_left,
                            rgb_pixels + ((memMoveRow + row_offset) * image_info->scaler_config.iOutBufWidth) + delta,
                            pixelsToMove);
                }
            }
    ...
    else
    {
        ...
        int dbytes = _decode_stripe(image_info,
                                    unpadded_start_row,
                                    (((unpadded_start_row + num_rows) > _get_height(image_info, PAD_NONE)) ? 
                                        (_get_height(image_info, PAD_NONE) - unpadded_start_row) : num_rows),
    ...
    if ((image_info->padding_options & PAD_BOTTOM) &&
        (num_rows > 0) &&
        (start_row >= _get_height(image_info, image_info->padding_options & PAD_TOP))) {
        int blank_rows = MIN(num_rows, _get_height(image_info, image_info->padding_options) - start_row);
        int bytesToBlank = (blank_rows * bytes_per_row);
...
int wprint_image_compute_rows_to_cache( wprint_image_info_t *image_info )
{
    ...
    available_mem -= (wprint_image_get_output_buff_size(image_info) * image_info->concurrent_stripes);
    if (image_info->unscaled_rows != NULL)
    {
        /* remove any memory allocated for scaling from our pool */
        available_mem -= BYTES_PER_PIXEL(image_info->unscaled_rows_needed * image_info->output_width);
    ...
    image_info->wprint_ifc->debug(DBG_LOG, "wprint_image_compute_rows_to_cache(): %d bytes available for row caching", available_mem);
    ...
    image_info->wprint_ifc->debug(DBG_LOG, "wprint_image_compute_rows_to_cache(): based on row width %d (%d), %d rows can be cached", row_width, output_mem, max_rows);
    ...
    image_info->wprint_ifc->debug(DBG_LOG, "wprint_image_compute_rows_to_cache(): %d rows being cached", max_rows);
